import{_ as n,U as s,a5 as a,a8 as t}from"./framework-a4fc28af.js";const p={},e=t(`<h1 id="归并排序" tabindex="-1"><a class="header-anchor" href="#归并排序" aria-hidden="true">#</a> 归并排序</h1><p>归并排序是利用归并的思想实现的排序方法，该算法采用经典的分治策略（分治法将问题分成一些小问题然后递归求解，而治的阶段则将分的阶段得到的各个答案“修补”在一起）。</p><h2 id="基本思想" tabindex="-1"><a class="header-anchor" href="#基本思想" aria-hidden="true">#</a> 基本思想</h2><p>将一个需要排序的数组通过递归进行拆分，当每一个元素都是一个个体的时候，再进行合并，合并时将合并的数据保存到一个临时开辟的空间中，这意味着需要额外的空间来保存数据；这个算法主要分为三步： （一）：拆分 将一个初始的数组递归拆分，将每一个元素拆分至单个个体独立存在（这里并不对数据进行操作） （二）：合并（核心） 将拆分的当个个体元素进行合并，合并的过程中进行排序，并保存到临时创建的空间内（需要对数据进行排序操作） （三）：拷贝 将临时空间内的数据拷贝到原数组中</p><h2 id="合并的实现图解如下" tabindex="-1"><a class="header-anchor" href="#合并的实现图解如下" aria-hidden="true">#</a> 合并的实现图解如下</h2><p><img src="https://qiniu.yanggl.cn/image/20191008164202611.png" alt="" loading="lazy"><img src="https://qiniu.yanggl.cn/image/20191008164215964.png" alt="" loading="lazy"></p><h2 id="代码实现" tabindex="-1"><a class="header-anchor" href="#代码实现" aria-hidden="true">#</a> 代码实现</h2><div class="language-java line-numbers-mode" data-ext="java"><pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">com<span class="token punctuation">.</span>ygl<span class="token punctuation">.</span>sort</span><span class="token punctuation">;</span>
