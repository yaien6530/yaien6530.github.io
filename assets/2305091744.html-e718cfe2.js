import{_ as s,o as n,c as e,e as a}from"./app-b93dd20e.js";const i={},l=a(`<h1 id="redis集群-cluster" tabindex="-1"><a class="header-anchor" href="#redis集群-cluster" aria-hidden="true">#</a> Redis集群（Cluster）</h1><p>redis集群是一个又多个<strong>主从节点群</strong>组成的<strong>分布式服务器群</strong>，它具有<strong>复制、高可用和分片</strong><br> 的特性。redis集群不需要sentinel哨兵也能完成节点移除和故障转移的功能。需要将每个节点设置成集群模式，这种集群模式没有中心节点，可水平扩展。根据官方文档称，可线性扩展到上万个节点（推荐不超过1W个节点）。redis集群的性能和高可用性均优于**<br> 哨兵模式**，且配置简单。</p><h2 id="搭建" tabindex="-1"><a class="header-anchor" href="#搭建" aria-hidden="true">#</a> 搭建</h2><p>redis集群搭建<strong>至少需要三个master</strong>节点，搭建的每个master再搭建一个或多个slave节点。每个主从节点之间会形成一个小的节点集群，小的节点集群也会进行主节点的选举。</p><blockquote><p>注意：集群的slave不支持读写，为了方便水平扩展，集群所有的读写都是通过master节点来完成的，slave节点仅用于备份数据，在master挂掉之后进行选举新的master节点</p></blockquote><ol><li>在第一台机器的 /usr/local目录下创建文件夹redis-cluster，然后在其下面分别创建2个文件夹<div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token function">mkdir</span> -p/usr/local/redis-cluster
<span class="token function">mkdir</span> <span class="token number">8001</span> <span class="token number">8004</span>
</code></pre></div></li><li>把redis-cof配置文件copy到8001下，并修改成以下内容：<div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># ./redis.conf</span>
...
masterauth xxx
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li><li>把修改好的配置文件，copy到8004，修改port、dir、clster-config-file里的端口</li><li>另外两台机器重复上面3步</li><li>分别启动6个redis实例</li><li>用reids-cli创建整个redis集群（以前的版本集群是依靠ruby脚本redis - trib.rb实现）</li></ol><h2 id="分片策略" tabindex="-1"><a class="header-anchor" href="#分片策略" aria-hidden="true">#</a> 分片策略</h2>`,7),r=[l];function c(d,t){return n(),e("div",null,r)}const p=s(i,[["render",c],["__file","2305091744.html.vue"]]);export{p as default};
